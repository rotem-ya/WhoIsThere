...
class ContentManifestService {
  ...
  // Remote places that are active AND whose image is available in the cache.
  final List<GameImageModel> _remoteAvailable = [];
  bool _loaded = false;
  ...
  /// Remote places ready to play (active + image cached). Copy, never null.
  List<GameImageModel> availableRemoteImages() =>
      List<GameImageModel>.unmodifiable(_remoteAvailable);
  ...
  Future<void> loadCached() async {
    try {
      ...
      QaLoggerService.instance.log('CONTENT',
          'MANIFEST_CACHE_LOADED places=${places.length} remoteReady=${_remoteAvailable.length}');
    } catch (e) {
  ...
  Future<void> sync() async {
    try {
      ...
      _loaded = true;
      QaLoggerService.instance.log('CONTENT',
          'MANIFEST_SYNCED places=${places.length} remoteReady=${_remoteAvailable.length} v=${(data['version'] ?? '?')}');
    } catch (e) {
