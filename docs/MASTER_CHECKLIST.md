...
### מטלה חדשה (2026-07-06): יעד המשוב והלוגים האוטומטיים · 🤖
**מה כבר קיים (מומש):** משוב מהפרופיל → Firestore ‏`feedback/{id}`; קריסות/שגיאות אוטומטיות (עם הלוג האחרון מהמכשיר) → Firestore ‏`crash_reports/{id}`; צפייה וטיפול במסך "📬 משוב וקריסות" באדמין (שלב 8).
**מה נשאר להחליט/לממש:**
- [ ] **התראה על משוב חדש** — כרגע רואים רק כשנכנסים לאדמין. אופציות: התראת דפדפן מהאדמין / מייל / בדיקה יזומה. (פוש אמיתי ידרוש Blaze+Functions.)
- [ ] **דוח האבחון של האדמין** — היום מועתק ידנית לצ'אט; לשקול כפתור "שלח דוח" שכותב ל-Firestore (`admin_reports/{id}`) כדי שקלוד/רותם יוכלו לקרוא בלי העתק-הדבק.
- [ ] **ניקוי/תחזוקה** — מדיניות מחיקה לפריטים מטופלים ישנים (חיסכון במקום; אולי ניקוי אוטומטי בכניסה לאדמין).
- [ ] **לוג QA מהמשחק לפי דרישה** — היום נשלח רק כשיש קריסה; לשקול צירוף אוטומטי של הלוג גם למשוב רגיל (עזרה באבחון תלונות "לא עובד לי").
