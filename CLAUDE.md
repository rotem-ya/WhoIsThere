...
## 🚀 סטטוס חנויות
### v1.2.0 — חי בשתי החנויות (מאושר, נכון ל-2026-07-11)
משחק "זהו את הפתגם". זו הגרסה הבסיסית ש-v1.3.0 מתעדכנת ממנה.

### v1.3.0 — בילד מוכן, ממתין להגשת cowork (2026-07-11)
חוויית חברים מלאה (קבוצות, פוש, אישור הצטרפות, טבלת ניקוד, תחבולות, קישור ישיר). פרומפט הגשה מלא: **`docs/COWORK_V130_SUBMIT.md`**. תג `ios-v12` + marker AAB נדחפו. מכיוון ש-v1.2.0 כבר חי ומאושר, זו הגשת עדכון פשוטה, בלי דילמת Remove-from-Review.
...
- **הזמנות משחק בפוש+באנר:** ה-cloud function `onGameInvite` (functions/index.js) כבר שולח פוש על כתיבת `gameInvites/{toUid}_{fromUid}`. חדש: `GameInviteBanner` גלובלי (main.dart, לצד FriendRequestBanner) עם "הצטרף" שמצרף ופותח לובי בלחיצה מכל מסך; לחיצה על הפוש עצמו עושה join ישיר לחדר (`_joinFromPush` ב-main, עם המתנה ל-auth בקולד-סטארט ונפילה ל-/friends). ✅ **הפוש פרוס וחי (2026-07-11):** Blaze מופעל, `firebase-adminsdk-fbsvc@whoisthere-380fa` קיבל תפקיד Editor ב-IAM, ושלוש הפונקציות (`onGameInvite`/`onFriendRequest`/`onGroupInvite`) פרוסות דרך `.github/workflows/deploy-functions.yml` — כל שינוי עתידי ב-`functions/` נפרס אוטומטית בפוש. ה-runtime הוא `nodejs20` (הוצא nodejs18, גוגל הפסיקו לתמוך בו ב-2025-10-30).
...
- [x] **חוויית חברים — מומש (2026-07-11):** ראה סעיף "חוויית חברים" למטה. functions פרוסות וחיות (Blaze מופעל, ראה למעלה). נותר: בדיקה חיה ממכשירים (רותם).
